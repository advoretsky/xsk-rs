...
fn run_server_loop(
    ...
    let umem_cfg = UmemConfig::default();
    let fr_ring_size = umem_cfg.fill_queue_size().get();
    let cr_ring_size = umem_cfg.comp_queue_size().get();
...
fn run_receiver(path: PathBuf) {
    ...
    println!(
        "receiver: got bundle: frames={} rx={} tx={} fr={} cr={} fds={:?}",
        payload.frame_count,
        payload.rx_ring_size,
        payload.tx_ring_size,
        payload.fr_ring_size,
        payload.cr_ring_size,
    ...
    let sock_cfg = SocketConfig::builder()
        ...
        .build();
    let fr_size =
        xsk_rs::config::QueueSize::new(payload.fr_ring_size).expect("fr ring size");
    let cr_size =
        xsk_rs::config::QueueSize::new(payload.cr_ring_size).expect("cr ring size");

    let (mut tx_q, rx_q, fq_cq) = unsafe {
        Socket::from_raw_fd(sock_cfg, &umem, socket_fd, fr_size, cr_size)
            .expect("Socket::from_raw_fd")
    };
